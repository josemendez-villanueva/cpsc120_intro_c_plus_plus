...
// Lab 06-01
//
// Made a program to make a random array and output its minimum value
//
...
void PrintArray(const array<int, number_elements>& the_array) {
  ...
  for (const int& elements : the_array){
    cout << elements << "\n";
  }
...
int FindMinimum(const array<int, number_elements>& the_array) {
  ...
  int minimum = the_array.at(0);
  for (int i = 0; i < number_elements; i++){
    try{
      if (the_array.at(i) < minimum){
        minimum = the_array.at(i);
      }
    }catch(const exception& e){
      ErrorMessage("Your minimum values is not being displayed properly");
      exit(1);
    }
  }
...
if (minimum_number <= 0 || maximum_number <= 0 || maximum_number < minimum_number){
    ErrorMessage("Your input is incorrect");
    exit(1);
  }
