...
// Lab 07-03
//
// Made a program to use a vector to calculate the average
//
...
void PrintVector(const vector<int>& the_vector) {
  // print out each element in the vector using an ostream_iterator
  try{
  ostream_iterator <int> out_it(cout, "\n");
  copy(the_vector.begin(), the_vector.end(), out_it);
  } catch(const exception& e){
    ErrorMessage("Print Vector Error");
  }
...
void FillVector(vector<int>& the_vector, const int nelements,
  ...
  // assign a random number to each location in the vector using
...
float CalculateAverage(const vector<int>& the_vector) {
  float average = 0;
  float size = the_vector.size();
  float sum = accumulate(the_vector.begin(), the_vector.end(), 0);
  average = sum / size;
