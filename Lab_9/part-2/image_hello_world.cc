...
// Lab 09-02
//
// Made a program to make an image using three colors and nested loops.
//
...
int main(int argc, char* argv[]) {
  ...
  for (float row = (image.height() - 1); row >= 0; row--){
    for (float column = 0; column < image.width(); column++){
      double red = row / (image.width() - 1) ;
      double green = column / (image.height() - 1);
      double blue = 0.25;
      int i_red = lround(255.0 * red);
