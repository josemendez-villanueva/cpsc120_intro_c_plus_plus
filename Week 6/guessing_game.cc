...
// Lab 06-03
//
// Made a program to take input from user to try and guess a secret card
//
...
int GatherGuess() {
  ...
  int guess_integer = 0;
  try{
    guess_integer = stoi(guess);
  }catch(const exception& e){
    ErrorMessage("Your guess input is incorrect");
  }
...
void WrongGuessMessage() {
  ...
  array<string, 4> responses = {"\t No way Jose \n", "\t Wrong number \n", "\t Try again \n", "\t Guess again \n"};
  RandomNumberGenerator rng(0, 3);
  int message_id = rng.next();
  try{
    cout << responses.at(message_id);
  }catch (const exception& e){
    ErrorMessage("Your Wrong Guess Message went crazy");
  }
